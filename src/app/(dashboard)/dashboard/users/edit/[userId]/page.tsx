...
export default function EditUserPage() {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const res = await uploadImageToCloudinary(file, 'users/avatars')
  ...
  const handleAadharUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const res = await uploadImageToCloudinary(file, 'users/documents/aadhar')
    setAadharLoading(false)
    if (res.success && res.data?.secure_url) {
      setFormData(prev => ({ ...prev, aadharimg: res.data!.secure_url }))
      toast.success('Aadhar document uploaded!')
    } else {
      toast.error(res.error || 'Failed to upload aadhar document')
    }
  }
  ...
  const handleDLUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setDlLoading(true)
    const res = await uploadImageToCloudinary(file, 'users/documents/dl')
    setDlLoading(false)
    if (res.success && res.data?.secure_url) {
      setFormData(prev => ({ ...prev, dlimg: res.data!.secure_url }))
      toast.success('Driver license uploaded!')
    } else {
      toast.error(res.error || 'Failed to upload driver license')
    }
  }
  ...
  const handlePassportUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const res = await uploadImageToCloudinary(file, 'users/documents/passport')
    setPassportLoading(false)
    if (res.success && res.data?.secure_url) {
      setFormData(prev => ({ ...prev, passportimg: res.data!.secure_url }))
      toast.success('Passport uploaded!')
    } else {
      toast.error(res.error || 'Failed to upload passport')
    }
  }
